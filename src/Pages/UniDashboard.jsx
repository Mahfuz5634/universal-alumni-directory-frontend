...
const UniDashboard = () => {
  ...
  useEffect(() => {
    fetchDirectory({ university_id: user.university_id }).then((res) =>
      setAlumni(res.data),
    );
  }, []);
  ...
  const handleVerify = async (id) => {
    ...
    setAlumni(
      alumni.map((a) => (a._id === id ? { ...a, is_verified: true } : a)),
    );
  };
            ...
            {alumni.map((person) => (
              <tr key={person._id} className="border-t">
                ...
                <td className="p-4">
                  {!person.is_verified && (
                    <button
                      onClick={() => handleVerify(person._id)}
