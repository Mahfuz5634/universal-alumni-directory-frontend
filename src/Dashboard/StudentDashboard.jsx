      ...
      >
        ...
        <div className={`p-6 flex items-center justify-between md:justify-start gap-3 border-b border-slate-100 ${isSidebarCollapsed ? "md:justify-center px-4" : ""}`}>
          <div className="flex items-center gap-3">
            ...
            <h1 className={`font-bold text-lg tracking-tight text-slate-900 leading-tight transition-opacity duration-200 whitespace-nowrap overflow-hidden ${isSidebarCollapsed ? "md:hidden" : "block"}`}>
              Alumni Portal
            </h1>
          </div>
          <button 
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto overflow-x-hidden">
          <p className={`px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 transition-opacity whitespace-nowrap ${isSidebarCollapsed ? "md:hidden" : "block"}`}>
            Navigation
          </p>
          ...
          >
            <div className={`flex items-center ${isSidebarCollapsed ? "" : "gap-3"}`}>
              <User className={`w-5 h-5 shrink-0 ${activeTab === "profile" ? "text-indigo-600" : "text-slate-400"}`} />
              <span className={`whitespace-nowrap ${isSidebarCollapsed ? "md:hidden" : "block"}`}>My Profile</span>
            </div>
            {activeTab === "profile" && !isSidebarCollapsed && <div className="w-1.5 h-1.5 bg-indigo-600 rounded-full shrink-0"></div>}
          </button>
          ...
          >
            <div className={`flex items-center ${isSidebarCollapsed ? "" : "gap-3"}`}>
              <Users className={`w-5 h-5 shrink-0 ${activeTab === "directory" ? "text-indigo-600" : "text-slate-400"}`} />
              <span className={`whitespace-nowrap ${isSidebarCollapsed ? "md:hidden" : "block"}`}>Directory</span>
            </div>
            {activeTab === "directory" && !isSidebarCollapsed && <div className="w-1.5 h-1.5 bg-indigo-600 rounded-full shrink-0"></div>}
          </button>
        </nav>

        <div className={`p-4 border-t border-slate-200 bg-slate-50/50 transition-all ${isSidebarCollapsed ? "md:px-2" : ""}`}>
          {user && (
            <div className={`flex items-center gap-3 mb-4 px-2 ${isSidebarCollapsed ? "md:justify-center" : ""}`}>
              ...
              <div className={`flex-1 min-w-0 ${isSidebarCollapsed ? "md:hidden" : "block"}`}>
                <p className="text-sm font-semibold text-slate-900 truncate" title={user.name}>{user.name || "Student"}</p>
          ...
          >
            ...
            <span className={`whitespace-nowrap ${isSidebarCollapsed ? "md:hidden" : "block"}`}>Sign Out</span>
          </button>
        </div>
      </aside>
          ...
          {activeTab === "profile" && (
            <div className="p-4 md:p-8 lg:px-12 lg:py-10 max-w-5xl mx-auto animate-in fade-in duration-300">
              <header className="mb-8 pt-2 md:pt-0">
                <h2 className="text-2xl font-bold text-slate-900">Personal Dashboard</h2>
                <p className="text-slate-500 mt-1 text-sm">Manage your academic identity and directory presence.</p>
              </header>
                      ...
                      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-slate-100 pt-8">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-slate-900 leading-tight">{universityName}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-slate-900 leading-tight">{user.department || "Not Specified"}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-0.5">Graduation</p>
                            <p className="text-sm font-medium text-slate-900 leading-tight">Class of {user.graduation_year || "N/A"}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                     ...
                     >
                       <div className="flex gap-4 mb-4">
                         ...
                         <div className="flex-1 min-w-0">
                           <h4 className="font-bold text-slate-900 text-base truncate" title={alumni.name}>
                             {alumni.name}
                           </h4>
                       ...
                       <div className="mb-5 flex-1 space-y-2">
                         <div className="flex items-center gap-2 text-sm text-slate-700">
                           ...
                           <span className="truncate" title={alumni.position || "Position not specified"}>
                             {alumni.position || "Professional"}
                           </span>
                         </div>
                         <div className="flex items-center gap-2 text-sm text-slate-700">
                           <Building2 className="w-4 h-4 text-slate-400 shrink-0" />
                           <span className="truncate" title={alumni.company || "Company not specified"}>
